...
## 테스트 격리 가능성으로 책임 경계 확인

파일 크기나 컴포넌트 종류가 아니라 독립된 행동과 정책을 기준으로 분리한다. 테스트 격리 가능성은
테스트 코드를 쉽게 작성하기 위한 편의가 아니라, 다른 기능을 함께 실행하지 않고 해당 책임만 검증할 수
있는지 확인하는 설계 도구다.

분리 전후에 다음을 확인한다.

- 이 단위에 독립적으로 입력을 줄 수 있는가
- 결과나 사용자 행동을 이 단위만 렌더링하거나 호출해서 관찰할 수 있는가
- API, router, 부모 Page 같은 관계없는 환경을 준비하지 않아도 되는가
- 실패했을 때 깨진 책임을 이 단위 안으로 한정할 수 있는가
- mock 대상이 이 단위의 직접적인 외부 의존성으로 제한되는가
- 별도로 검증할 행동이나 정책이 실제로 존재하는가

파일만 옮기고 부모의 거대한 style 객체, query 결과, mutation이나 암묵적 전역 상태에 계속 의존하면
독립된 unit으로 보지 않는다. 필요한 값과 callback은 해당 책임을 드러내는 입력으로 받되, props가
계속 늘어나면 여러 책임이 섞였는지 먼저 재검토한다.

독립된 이름과 UI 책임을 가진 React 컴포넌트는 크기와 관계없이 컴포넌트별 파일로 분리하고 전용 style과
type을 함께 둔다. 이름 없는 단순 마크업까지 예상 재사용을 위해 컴포넌트로 추출하거나 한 소비자만 쓰는
자명한 helper를 상위 디렉터리로 옮기지는 않는다. Page에는 route parameter 해석, 페이지 수준 상태,
독립 unit 조립과 페이지 전환처럼 Page에서만 의미 있는 동작을 남긴다.
...
- 독립된 행동이나 정책을 다른 기능 없이 단독 검증할 수 있는가
- 파일 이동만 했을 뿐 부모 Page의 상태, side effect 또는 style에 계속 결합되어 있지 않은가
- 독립된 이름과 UI 책임을 가진 컴포넌트가 Page 파일 안에 함께 선언되어 있지 않은가
- 이름 없는 단순 마크업이나 한 소비자만 쓰는 helper까지 예상 재사용을 위해 추출하지 않았는가
