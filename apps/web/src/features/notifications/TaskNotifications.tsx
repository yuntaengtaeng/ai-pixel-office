...
export function TaskNotifications({ workspaceId }: { workspaceId: string }) {
  ...
  const notificationPermission = useNotificationPermission(announce);
  ...
  return (
    <Styled.Region aria-live="polite" aria-label="작업 상태 알림">
      {notificationPermission.permission === "default" && notificationPermission.promptVisible && (
        <Styled.Snackbar $tone="info">
          <div>
            <strong>백그라운드 작업 알림을 받을까요?</strong>
            <span>다른 창을 보고 있을 때 검토나 입력이 필요하면 알려드려요.</span>
            {notificationPermission.hint && (
              <Styled.PermissionHint>{notificationPermission.hint}</Styled.PermissionHint>
            )}
          </div>
          <Styled.ActionButton
            type="button"
          ...
          >
            {notificationPermission.requesting ? "권한 확인 중..." : "알림 켜기"}
          </Styled.ActionButton>
          <Styled.CloseButton
            type="button"
            onClick={notificationPermission.dismissPrompt}
