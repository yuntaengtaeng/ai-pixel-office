export type PetAnimationStatus =
  | "idle"
  | "working"
  | "needs_review"
  | "needs_input"
...
export type PetAnimation = {
  frame(input: PetAnimationInput): PetAnimationFrame;
};

const baseFrame = (): PetAnimationFrame => ({ x: 0, y: 0, rotation: 0, scaleX: 1, scaleY: 1 });

export function getPetAnimation(petId: string): PetAnimation {
  return { frame: (input) => petFrame(petId, input) };
}

function petFrame(petId: string, input: PetAnimationInput): PetAnimationFrame {
  const phase = input.phase ?? 0;
  const wave = Math.sin(input.time + phase);
  const frame = baseFrame();
  if (petId === "rabbit-yuzu") return yuzuFrame(frame, input, phase, wave);
  if (petId === "capybara-gamja") return gamjaFrame(frame, input, phase);
  if (petId === "quokka-bangul") return bangulFrame(frame, input, phase);
  return defaultFrame(frame, input, phase, wave);
}

function yuzuFrame(frame: PetAnimationFrame, input: PetAnimationInput, phase: number, wave: number) {
  if (input.status === "working") {
    ...
    frame.rotation = -0.04;
  } else frame.y = -Math.round(Math.abs(Math.sin(input.time * 0.8 + phase)) * 3);
  return frame;
}

function gamjaFrame(frame: PetAnimationFrame, input: PetAnimationInput, phase: number) {
  frame.scaleY = 1 + Math.sin(input.time * 0.35 + phase) * 0.015;
  if (input.status === "working") frame.y = Math.round(Math.sin(input.time * 1.2 + phase));
  else if (input.status === "done") {
...
function bangulFrame(frame: PetAnimationFrame, input: PetAnimationInput, phase: number) {
  frame.rotation = Math.sin(input.time * 0.9 + phase) * 0.025;
  if (input.status === "working") frame.y = -Math.round(Math.abs(Math.sin(input.time * 2 + phase)) * 3);
  else if (input.status === "done") {
...
function defaultFrame(frame: PetAnimationFrame, input: PetAnimationInput, phase: number, wave: number) {
  if (input.status === "working") {
